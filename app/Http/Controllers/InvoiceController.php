						...
						if (Gate::allows('invoice_edit') || Gate::allows('invoice_delete')) {
							$actionDropdown .= '<li>
								<button type="button" 
									...
									<img src="' . asset('public/img/list/payment.png') . '" class="me-3">' . 
						...
						if (auth()->user()->can('invoice_view')) {
							if ($invoice->type == 0) {
								$actionDropdown .= '<li>
									...
									target="_blank" 
									class="dropdown-item">
										<img src="' . asset('public/img/list/WhatsApp.png') . '" class="me-2">' . 
					...
					elseif (getUserRoleFromUserTable(Auth::User()->id) == 'Customer') {
						...
						if (auth()->user()->can('invoice_view')) {
							...
							if ($amountdue != 0 && $amountdue < 999999 && $updatekey->publish_key != null) {
								...
								$actionDropdown .= '<form method="get" action="' . url('stripe/checkout') . '" class="medium" id="medium">
								...
								$actionDropdown .= '<li>
									<input type="submit" class="dropdown-item payWarning" value="' . trans('message.Pay') . '" />
								</li>';
							} else {
								if ($invoice->grand_total > $invoice->paid_amount) {
									$actionDropdown .= '<li>
				...
				if ($emailformats->is_send == 0) {
					if ($invoice->save()) {
						...
						Mail::send([], [], function ($message) use ($email, $mail_sub, $blade_view, $mail_send_from,$filePath,$invoice_pdf) {
						 ...
						 Log::info('Service invoice email sent to: ' . $email);
