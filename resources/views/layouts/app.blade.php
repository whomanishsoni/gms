          ...
          <div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
            <div class="menu_section">
              <ul class="nav side-menu">
                ...
                @else
                @can('timezone_view')
                <li class="{{in_array($currentRoute,$inventoryRoutes) ? 'active' : ''}}"><a data-toggle="" data-placement="top" href="{!! url('/setting/timezone/list') !!}" title="{{trans('message.Settings')}}"> <i class="fa-solid fa-gear margin-right-10px"></i>{{trans('message.Settings')}}</a></li>
                @endcan
                @endif
                @endif
                <li>
      ...
      <div class="top_nav position-relative">
       ...
       $systemName = Cache::get('system_name', function () {
        ...
        <footer class="footerforallpage bottom-0 bg-white text-center" id="footerforid">
         <span class="footerbottom me-3">{{ trans('message.Copyright') }} {{ $year }} | {{ $systemName }} | {{ trans('message.All Rights Reserved') }} | {{trans('message.Version')}} {{$version}}</span>
        </footer>                        
      </div>
    </div>
  </div>
